...
var (
	...
	ErrGfxNoDeviceInfoFound        = fmt.Errorf("no graphic devcie info found")
	ErrDTRdspLenOverBound          = fmt.Errorf("rdsp table length too large")
...
func buildDtMemoryNode(mem *kexec.Memory) []*dt.Node {
	...
	for _, ram := range ramRanges {
		...
		node := dt.NewNode(nodeName, dt.WithProperty(
			dt.PropertyRegion("reg", uint64(ram.Start), uint64(ram.Size)),
		))
...
func buildFrameBufferNode() (*dt.Node, error) {
	...
	return dt.NewNode(framebufferNodeName, dt.WithProperty(
		...
		dt.PropertyU32("redmask", ((1<<vinfo.Red.Length)-1)<<vinfo.Red.Offset),
		dt.PropertyU32("greenmask", ((1<<vinfo.Green.Length)-1)<<vinfo.Green.Offset),
		dt.PropertyU32("bluemask", ((1<<vinfo.Blue.Length)-1)<<vinfo.Blue.Offset),
		dt.PropertyU32("reservedmask", ((1<<vinfo.Transp.Length)-1)<<vinfo.Transp.Offset),
...
func GetDisplayDeviceInfo() ([]map[string]string, error) {
	...
	drmDevices, err := ioutil.ReadDir(sysfsDrmPath)
	if err != nil {
	...
	for _, dev := range drmDevices {
		...
		info, err := os.Stat(filepath.Join(sysfsDrmPath, deviceName))
		if !(info.IsDir()) {
			continue
		}
...
func buildGraphicNode() (*dt.Node, error) {
	...
	return dt.NewNode("Gma", dt.WithProperty(
		dt.PropertyU32("vendor-id", uint32(vendorID)),
		dt.PropertyU32("device-id", uint32(deviceID)),
		dt.PropertyU32("revision-id", uint32(revisionID)),
		dt.PropertyU32("subsystem-vendor-id", uint32(subVendorID)),
		dt.PropertyU32("subsystem-id", uint32(subsysID)),
	)), nil
}
...
func buildDeviceTreeInfo(buf io.Writer, mem *kexec.Memory, addr uint64) ([]byte, error) {
	...
	if rsdpLen > uint32(pageSize) {
		return nil, ErrDTRdspLenOverBound
	}
	...
	pciNode := dt.NewNode("pci-rb", dt.WithProperty(
		...
		dt.WithChildren(
			dt.NewNode("serial@", dt.WithProperty(
	...
	fmt.Printf("dtHeader struct length:%x, DtStruct:%x\n", unsafe.Sizeof(dt.Header{}), unsafe.Sizeof(dtRootNode))
